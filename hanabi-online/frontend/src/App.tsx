...
const SetupScreen: React.FC<SetupScreenProps> = ({ onStartGame, isLoading, error }) => {
    ...
    return (
        <div style={{ 
            ...
            {/* Відображення помилки (НОВИЙ БЛОК) */}
            {error && (
                <p 
                    role="alert" 
                    style={{ color: 'red', marginTop: '20px', marginBottom: '20px', fontWeight: 'bold' }}
                >
                    Error: {error}
                </p>
            )}
...
export default function App() {
    ...
    if (!gameId) {
        return (
            <SetupScreen onStartGame={startGame} isLoading={isLoading} error={error} />
        );
    }
