...
function setupGameLoop(game: HanabiRoom, io: Server) {
    game.onChange(async () => {
        ...
        if (!snapshot.finished && currentPlayer && currentPlayer.isBot) {
            try {
                await game.performBotMove(currentPlayer.id);
            } catch (error) {
...
    setupGameLoop(newGame, io);

    const snapshot = newGame.snapshot();
    const firstPlayer = snapshot.players[snapshot.currentPlayerIndex];
